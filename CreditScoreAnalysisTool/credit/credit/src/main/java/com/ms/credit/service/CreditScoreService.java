...
public class CreditScoreService {
     ...
     */
    public CreditScoreDTO getCreditScoreByEmailId(Long userId) {
    ...
    /**
     * Retrieves the latest credit score by user ID with JWT token authentication.
     */
    public CreditScoreDTO getCreditScoreByEmailId(Long userId, String jwtToken) {
        String emailId = userManagementClient.getUserDetails(userId, jwtToken).block();  // Synchronous call with JWT token.
        CreditScore creditScore = (CreditScore) redisTemplate.opsForValue().get(emailId);
        if (creditScore == null) {
            logger.info("Fetching from DB");
            creditScore = creditScoreRepository.findTopByUserIdOrderByDateDesc(userId);
    ...
    private void sendToKafka(Long score, String emailId) {
        NotificationDTO message = new NotificationDTO(null, null, "Credit score updated to: " + score, new Date());
     ...
     */
    public CreditScoreDTO getCreditScoreByUserId(Long userId) {
    ...
    /**
     * Calculates a new credit score with JWT token authentication, saves it to the database and cache.
     */
    public CreditScoreDTO calculateCreditScore(CreditScoreDTO creditScoreDTO, String jwtToken) {
        String emailId = userManagementClient.getUserDetails(creditScoreDTO.getUserId(), jwtToken).block();
        redisTemplate.opsForValue().set(emailId, convertToEntity(creditScoreDTO));
        CreditScore creditScore = creditScoreRepository.save(convertToEntity(creditScoreDTO));
        return convertToDTO(creditScore);
    }
     ...
     */
    public CreditScoreDTO updateCreditScore(Long userId, CreditScoreDTO creditScoreDTO) {
        ...
        existingScore.setDate(java.sql.Timestamp.valueOf(creditScoreDTO.getDate()));
     ...
     */
    public void deleteCreditScoreByUserId(Long userId) {
     ...
     */
    public List<ScoreHistoryDTO> getCreditScoreHistoryByUserId(Long userId) {
    ...
    private CreditScoreDTO convertToDTO(CreditScore creditScore) {
        return new CreditScoreDTO(creditScore.getUserId(), creditScore.getScore(), 
            creditScore.getDate() != null ? creditScore.getDate().toLocalDateTime() : LocalDateTime.now());
    }
    ...
    private CreditScore convertToEntity(CreditScoreDTO creditScoreDTO) {
        ...
        creditScore.setDate(java.sql.Timestamp.valueOf(creditScoreDTO.getDate()));
        return creditScore;
    }
    ...
    private ScoreHistoryDTO convertToScoreHistoryDTO(CreditScore creditScore) {
        ...
        details.setDate(creditScore.getDate() != null ? creditScore.getDate().toLocalDateTime() : LocalDateTime.now());
